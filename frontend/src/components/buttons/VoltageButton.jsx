...
const VoltageButton = ({ children, to = null, onClick = null, type = "button" }) => {
  // The button content with all the SVG and animation elements
  const buttonContent = (
    <>
      {children}
  ...
  if (to) {
    return (
      <div className="voltage-button">
        <Link to={to} className="button-content">
          {buttonContent}
        </Link>
      </div>
    );
  ...
  return (
    <div className="voltage-button">
      <button type={type} onClick={onClick} className="button-content">
        {buttonContent}
      </button>
    </div>
  );
